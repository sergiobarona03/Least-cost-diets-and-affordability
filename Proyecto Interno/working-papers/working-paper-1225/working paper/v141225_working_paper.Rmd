...
directorio_base <- dirs[dir.exists(dirs)][1]

if (is.na(directorio_base)) {
...
sipsa_tcac <- readxl::read_excel(file.path(directorio_base,"composicion-nut/1823_mapeo_sipsa_tcac v1.0_2025.xlsx")) %>%
  janitor::clean_names() %>%
  rename(food_sipsa = alimento_nombre_sipsa)

# Cargar mapeo IPC - SIPSA
ipc_sipsa = readxl::read_excel(file.path(directorio_base,"composicion-nut\\Copia_DANE_4_DIC_2025act.xlsx")) %>%
  janitor::clean_names() %>% 
...
knitr::include_graphics(c(
  paste0(
    directorio_base,
    "/working-papers/working-paper-1225/m4/output/m4_ecm_dlog_grouped_page_01.png"
  )
))
```
...
knitr::include_graphics(c(
  paste0(
    directorio_base,
    "/working-papers/working-paper-1225/model_comparison/output/plot_box_mape_by_model.png"
  )
))
```
...
knitr::include_graphics(c(
  paste0(
    directorio_base,
    "/working-papers/working-paper-1225/model_comparison/output/ipc_plot_heatmap_mape_food_model.png"
  )
))
```
...
knitr::include_graphics(c(
  paste0(
    directorio_base,
    "/working-papers/working-paper-1225/model_comparison/output/sipsa_plot_heatmap_mape_food_model.png"
  )
))
```
...
Los resultados indican que la Metodología I, basada en la variación mensual del IPC a nivel de subclase, presenta el MAPE mediano más bajo entre todas las metodologías evaluadas, y su distribución de errores es la menos dispersa. Este resultado valida su uso para estimar precios minoristas de alimentos en períodos no cubiertos por los datos originales del DANE. El heatmap de precisión muestra que la mayoría de los alimentos se clasifican en las categorías excelente (MAPE menor a 5%) y bueno (MAPE entre 5% y 10%), con errores elevados concentrados en un subconjunto específico de alimentos de alta volatilidad. Adicionalmente, la Metodología I no requiere información de precios mayoristas de SIPSA, lo que la hace aplicable a cualquier ciudad y período para los que se disponga de información del IPC a nivel de subclase, sin depender del mapeo IPC-SIPSA ni de sus limitaciones asociadas.

La comparación con las demás metodologías refuerza esta conclusión. Las Metodologías II a VI, basadas en precios mayoristas de SIPSA, presentan distribuciones de MAPE con mayor dispersión y valores extremos más pronunciados, visibles en el boxplot de validación. En particular, la Metodología VI muestra valores atípicos que superan los 2000%, lo que indica problemas de predicción para alimentos específicos. En todos los casos, los perecederos frescos como aguacate, repollo y lechuga se ubican consistentemente en la categoría de precisión pobre (MAPE mayor a 20%) con independencia de la metodología empleada, lo que refleja una limitación de los datos y no de los métodos.
