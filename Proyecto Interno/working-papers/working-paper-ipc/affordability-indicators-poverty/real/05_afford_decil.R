...
plot_heat_continuous <- function(df, x_var, fill_var, title_text, file_name,
  ...
  p <- ggplot(df, aes(x = {{x_var}}, y = factor(decil_num), fill = {{fill_var}})) +
    ...
    labs(title=title_text, x=NULL, y="Decile") +
    theme_heat_paper
...
afford_m <- readRDS(file.path(afford_metrics_dir,"Afford_city_month.rds")) %>%
  mutate(
    ...
    decil_num = as.integer(parse_number(deciles)),
...
plot_heat_continuous(
  afford_m_full %>% filter(model=="CoCA"),
  mes, rate_plot,
  "CoCA incidence by decile (monthly)",
  "CoCA_decil_month.png"
)

# Monthly CoNA
plot_heat_continuous(
  afford_m_full %>% filter(model=="CoNA"),
  mes, rate_plot,
  "CoNA incidence by decile (monthly)",
  "CoNA_decil_month.png"
)

# Monthly CoRD
plot_heat_continuous(
  afford_m_full %>% filter(model=="CoRD"),
  mes, rate_plot,
  "CoRD incidence by decile (monthly)",
  "CoRD_decil_month.png"
)
...
# Quarterly CoCA
plot_heat_continuous(
  afford_q_full %>% filter(model=="CoCA"),
  tri_date, rate_plot,
  "CoCA incidence by decile",
  "CoCA_decil_quarter.png"
)

# Quarterly CoNA
plot_heat_continuous(
  afford_q_full %>% filter(model=="CoNA"),
  tri_date, rate_plot,
  "CoNA incidence by decile",
  "CoNA_decil_quarter.png"
)

# Quarterly CoRD
plot_heat_continuous(
  afford_q_full %>% filter(model=="CoRD"),
  tri_date, rate_plot,
  "CoRD incidence by decile",
  "CoRD_decil_quarter.png"
)
