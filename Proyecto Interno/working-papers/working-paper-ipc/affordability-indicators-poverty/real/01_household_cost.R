...
afford_dir <- file.path(base_dir, "working-papers/working-paper-ipc/output/affordability/real")
...
message("Subgroups en serv_tbl:")
print(sort(unique(serv_tbl$Subgroup)))

message("Subgroups en diverse_tbl:")
...
missing_diverse_labels <- needed_labels[!(norm_txt(needed_labels) %in% norm_txt(unique(diverse_tbl$Subgroup)))]
if (length(missing_diverse_labels) > 0) {
  stop("Mapping -> diverse tiene labels que NO existen en diverse_tbl$Subgroup: ",
       paste(missing_diverse_labels, collapse = ", "))
}
