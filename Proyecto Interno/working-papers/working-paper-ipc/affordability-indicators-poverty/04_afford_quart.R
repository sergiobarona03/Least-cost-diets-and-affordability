#######################################################################
## Afford TRIMESTRAL: Income trimestral + CoCA/CoNA/CoRD trimestral
#######################################################################
...
#----------------------------------------------------------------------
# 0) FIX: aseguramos que NO haya una versión vieja de collapse_to_person()
#----------------------------------------------------------------------
if (exists("collapse_to_person", envir = .GlobalEnv)) {
  rm(list = "collapse_to_person", envir = .GlobalEnv)
}

# FIX mínimo ROBUSTO (sin across): deja 1 fila por Person
collapse_to_person <- function(df) {
  if (!("Person" %in% names(df))) return(df)
  
  num_cols <- names(df)[sapply(df, is.numeric)]
  num_cols <- setdiff(num_cols, "Person")
  
  out <- stats::aggregate(
    df[, num_cols, drop = FALSE],
    by = list(Person = df$Person),
...
if (!("trimestre" %in% names(cord_df))) cord_df <- cord_df %>% mutate(trimestre = paste0(year, "Q", q))
...
if (!("per_capita" %in% names(cord_df))) {
  if ("per_capita_month" %in% names(cord_df)) {
    cord_df <- cord_df %>% mutate(per_capita = per_capita_month / 30.4167)
  } else {
    stop("CoRD trimestral no tiene 'per_capita' ni 'per_capita_month'. Revisa columnas.")
  }
}
...
for (city.x in city_vector) {
  for (tri.x in tri_vector) {
    ...
    # FIX mínimo: pasar de 9 filas (Person repetido) a 3 filas (Person 1-3)
    coca_aux <- collapse_to_person(coca_aux)
    cona_aux <- collapse_to_person(cona_aux)
    cord_aux <- collapse_to_person(cord_aux)
...
ggsave(
  file.path(afford_metrics_dir, "Afford_Incidence_CoRD_cuartiles_city_time.png"),
  g_cord,
  width = 10,
  height = 5,
  dpi = 300,
...
panel_afford <- (g_coca / g_cona / g_cord) +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom")

ggsave(
  file.path(afford_metrics_dir, "Afford_Incidence_PANEL_cuartiles_city_time.png"),
  panel_afford,
  width = 10,
  height = 12,
  dpi = 300,
...
aff_inc_panel <- aff_inc %>%
  mutate(
    ciudad = factor(std_city(ciudad), levels = city_levels),
    model  = factor(model, levels = c("CoCA", "CoNA", "CoRD"))
  )

y_max_common <- max(aff_inc_panel$incidence, na.rm = TRUE)

# redondear hacia arriba al múltiplo de 10 más cercano
y_top <- ceiling(y_max_common / 10) * 10

common_y_scale <- scale_y_continuous(
  limits = c(0, y_top),
  breaks = seq(0, y_top, by = 10),
  labels = function(x) paste0(x, "%")
)

#------------------------------------------------------------
# Escala de colores para dietas
#------------------------------------------------------------
diet_scale <- scale_color_manual(
  values = c(
    "CoCA" = "#C23B22",
    "CoNA" = "#1F78B4",
    "CoRD" = "#E08D26"
...
# (a) Bogotá
#------------------------------------------------------------
g_bog_panel <- aff_inc_panel %>%
  filter(ciudad == "BOGOTA") %>%
  ggplot(aes(x = trimestre, y = incidence, color = model, group = model)) +
  geom_line(linewidth = 1) +
  common_y_scale +
  labs(
    title = "(a) Bogotá",
...
# (c) Medellín
#------------------------------------------------------------
g_med_panel <- aff_inc_panel %>%
  filter(ciudad == "MEDELLIN") %>%
  ggplot(aes(x = trimestre, y = incidence, color = model, group = model)) +
  geom_line(linewidth = 1) +
  common_y_scale +
  labs(
...
# PANEL FINAL
#------------------------------------------------------------
panel_afford_city_clean <- (g_bog_panel | g_cali_panel | g_med_panel) +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom")
...
#------------------------------------------------------------
ggsave(
  file.path(afford_metrics_dir, "Afford_Incidence_PANEL_city_time.png"),
  panel_afford_city_clean,
  width = 15,
  height = 5.2,
  dpi = 300,
  bg = "white"
