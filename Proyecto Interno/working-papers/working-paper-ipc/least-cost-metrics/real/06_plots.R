...
##  - out_dir/cona_cost_fullsample.rds  
##  - out_dir/cord_cost_fullsample.rds  
## Outputs:
...
plot_compare_cities_mean_cost_day <- function(df, metric) {
  d <- df %>%
    ...
    dplyr::summarise(mean_cost_day = mean(cost_day, na.rm = TRUE), .groups = "drop")
...
plot_comp_top_constraints_by_city <- function(comp_df, metric, out_png) {
  ...
  diag_df <- comp_df %>%
    dplyr::filter(ciudad %in% cities) %>%
    dplyr::count(ciudad, .data[[nutr_col]], sort = TRUE) %>%
