...
# 1) Load tables_12 
tables_path <- file.path("working-papers/working-paper-aecm/output-paper/tables/tables_12.rds")

if (!file.exists(tables_path)) {
  stop(paste0(
    "File not found: ", tables_path, "\n",
    "You need to save tables_12 first (see the save snippet I sent)."
  ))
}

tables_12 <- readRDS(tables_path)

# 2) Keep only these 4 foods
alimentos_keep <- c(
  "ARROZ PARA SECO",
  "CEBOLLA CABEZONA",
  "PAPA",
...
# 3) Print Table for each matching group 
nms <- names(tables_12)
if (is.null(nms)) stop("'tables_12' has no names().")

for (nm in nms) {

  if (any(str_detect(nm, alimentos_keep))) {

    cat("\n\n### Table 7 – ", nm, "\n\n", sep = "")

    if (!("table7" %in% names(tables_12[[nm]]))) {
      cat("_Warning: table7 not found for this group._\n\n")
      next
    }

    print(
      kable(tables_12[[nm]]$table7, booktabs = TRUE, digits = 3) %>%
...
# Cargar tabla
aecm_path <- file.path("working-papers/working-paper-aecm/output-paper/tables/aecm_results_12.rds")
if (!file.exists(aecm_path)) {
  stop(paste0("No encuentro: ", aecm_path,
              "\nAsegúrate de haber guardado el .rds con tus resultados A-ECM."))
}
aecm_list <- readRDS(aecm_path)

# ===== 2) Alimentos que quieres (los del IPC: articulo_ipc) =====
...
fmt_et <- function(b, t) {
  if (is.null(b) || is.null(t) || is.na(b) || is.na(t)) return(NA_character_)
  sprintf("%.4f (%.3f)", b, t)
}

# Construye 2 tablas: (A) EC terms, (B) F tests
make_aecm_tables <- function(fit) {

  # --- (A) ECM terms ---
  ecm <- fit$ecm_terms
  if (is.null(ecm)) stop("No existe fit$ecm_terms en este objeto.")

  need1 <- c("equation","regressor","estimate","t_value")
  miss1 <- setdiff(need1, colnames(ecm))
  if (length(miss1) > 0) stop(paste("Faltan columnas en ecm_terms:", paste(miss1, collapse=", ")))

  ecm$value <- mapply(fmt_et, ecm$estimate, ecm$t_value)

  eqs  <- unique(ecm$equation)
  regs <- unique(ecm$regressor)

  ecm_wide <- data.frame(equation = eqs, stringsAsFactors = FALSE)
  for (r in regs) ecm_wide[[r]] <- NA_character_

  for (i in seq_len(nrow(ecm))) {
    rr <- ecm$regressor[i]
    ee <- ecm$equation[i]
    ecm_wide[ecm_wide$equation == ee, rr] <- ecm$value[i]
  }

  # --- (B) F tests ---
  Ft <- fit$F_tests
  if (is.null(Ft)) stop("No existe fit$F_tests en este objeto.")

  need2 <- c("test","meaning","F","p")
  miss2 <- setdiff(need2, colnames(Ft))
  if (length(miss2) > 0) stop(paste("Faltan columnas en F_tests:", paste(miss2, collapse=", ")))

  Ft$`F (p)` <- sprintf("%.2f (%.3f)", Ft$F, Ft$p)
  Ft_out <- Ft[, c("test","meaning","F (p)")]

  list(ec = ecm_wide, F = Ft_out)
}

# Imprimir tabla (4 alimentos)
nms <- names(aecm_list)
if (is.null(nms)) {
  nms <- paste0("group_", seq_along(aecm_list))
  names(aecm_list) <- nms
}
...
for (nm in nms) {
  ...
  # filtro por alimento usando:
  # (i) obj$articulo_ipc si existe, o
  # (ii) el nombre "nm" si trae "articulo_ipc=..."
  obj <- aecm_list[[nm]]

  food_here <- NA_character_
  if (!is.null(obj$articulo_ipc)) food_here <- as.character(obj$articulo_ipc)
  if (is.na(food_here)) {

    m <- str_match(nm, "articulo_ipc=([^_|]+)")
    if (!is.na(m[1,2])) food_here <- m[1,2]
  }
  ...
  # título
  ttl <- nm
  if (!is.null(obj$name_key)) ttl <- pretty_key(obj$name_key)
  ttl <- pretty_key(ttl)
  ...
  fit <- obj$fit
  if (is.null(fit)) {
    cat("_Warning: este grupo no trae $fit._\n\n")
    next
  }

  # tau_x 
  if (!is.null(fit$tau_x)) {
    cat("**Umbral M-TAR (\\(\\tau_x\\)) = ", sprintf("%.5f", fit$tau_x), "**\n\n", sep = "")
  }

  tabs <- make_aecm_tables(fit)

  cat("**(A) Coeficientes de corrección de error asimétrica** (estimación (t))\n\n")
  print(
    kable(tabs$ec, booktabs = TRUE, align = "l") %>%
      kable_styling(full_width = FALSE, position = "center")
  )

  cat("\n\n**(B) Pruebas F sobre dinámica de corto plazo** (F (p))\n\n")
  print(
    kable(tabs$F, booktabs = TRUE, align = "l") %>%
      kable_styling(full_width = FALSE, position = "center")
  )
}
