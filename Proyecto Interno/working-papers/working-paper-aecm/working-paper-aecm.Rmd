...
Sea \(P^{W}_{i,c,t}\) el **precio mayorista** (SIPSA) del alimento \(i\) en la ciudad \(c\) y mes \(t\), y \(P^{R}_{i,c,t}\) el **precio minorista** (IPC/DANE) correspondiente. Trabajamos en logaritmos:
...
donde \(\mu_{i,c,t}\) es el residuo que captura el **desequilibrio** respecto al equilibrio de largo plazo. Si existe cointegración, \(\mu_{i,c,t}\) es estacionario.

El modelo TAR (Threshold Autoregressive) permite un ajuste asimétrico según el signo del desequilibrio:
...
donde \(\tau\) es el umbral. Siguiendo la práctica común en Enders y Siklos, suele fijarse \(\tau = 0\).

El sistema converge al equilibrio si \(\rho_1 < 0\), \(\rho_2 < 0\) y \((1+\rho_1)(1+\rho_2) < 1\). El ajuste es **simétrico** únicamente cuando \(\rho_1=\rho_2\), caso en el cual el modelo se reduce al ajuste lineal estándar de Engle--Granger.


### Modelo M-TAR

Una extensión del TAR es el modelo M-TAR (Momentum Threshold Autoregressive), donde el régimen depende de la **dirección del cambio** del desequilibrio en el periodo previo:
...
En aplicaciones empíricas suele fijarse \(\tau=0\), aunque también puede estimarse de manera consistente siguiendo el procedimiento de Chan (1993).

El modelo M-TAR permite capturar ajustes asimétricos asociados al \emph{momentum} del desequilibrio: por ejemplo, reacciones distintas cuando la brecha mayorista--minorista se está ampliando (\(\Delta\mu_{i,c,t-1}\ge 0\)) frente a cuando se está cerrando (\(\Delta\mu_{i,c,t-1}<0\)). La cointegración con ajuste M-TAR se verifica cuando \(\rho_1<0\) y \(\rho_2<0\).


```{r table7_only_4foods, echo=FALSE, warning=FALSE, message=FALSE, results="asis"}
library(stringr)
library(knitr)
library(kableExtra)
