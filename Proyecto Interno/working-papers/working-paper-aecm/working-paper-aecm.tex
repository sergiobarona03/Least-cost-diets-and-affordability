...
Sea \(P^{W}_{i,c,t}\) el \textbf{precio mayorista} (SIPSA) del alimento
\(i\) en la ciudad \(c\) y mes \(t\), y \(P^{R}_{i,c,t}\) el
\textbf{precio minorista} (IPC/DANE) correspondiente. Trabajamos en
logaritmos:

\begin{equation}
p^{W}_{i,c,t}=\ln(P^{W}_{i,c,t}), 
\qquad 
p^{R}_{i,c,t}=\ln(P^{R}_{i,c,t}).
\end{equation}
...
\begin{equation}
p^{W}_{i,c,t} = \alpha_0 + \alpha_1 p^{R}_{i,c,t} + \mu_{i,c,t},
\end{equation}

donde \(\mu_{i,c,t}\) es el residuo que captura el
\textbf{desequilibrio} respecto al equilibrio de largo plazo. Si existe
cointegración, \(\mu_{i,c,t}\) es estacionario.

El modelo TAR (Threshold Autoregressive) permite un ajuste asimétrico
según el signo del desequilibrio:

\begin{equation}
\Delta \mu_{i,c,t} 
= I_{i,c,t}\rho_1 \mu_{i,c,t-1} 
+ (1-I_{i,c,t})\rho_2 \mu_{i,c,t-1} 
+ \varepsilon_{i,c,t},
\end{equation}

con función indicadora tipo Heaviside:

\begin{equation}
I_{i,c,t}=
\begin{cases}
1 & \text{si } \mu_{i,c,t-1}\ge \tau,\\
0 & \text{si } \mu_{i,c,t-1}< \tau,
\end{cases}
\end{equation}

donde \(\tau\) es el umbral. Siguiendo la práctica común en Enders y
Siklos, suele fijarse \(\tau = 0\).

El sistema converge al equilibrio si \(\rho_1 < 0\), \(\rho_2 < 0\) y
\((1+\rho_1)(1+\rho_2) < 1\). El ajuste es \textbf{simétrico} únicamente
cuando \(\rho_1=\rho_2\), caso en el cual el modelo se reduce al ajuste
lineal estándar de Engle--Granger.

\subsubsection{Modelo M-TAR}\label{modelo-m-tar}

Una extensión del TAR es el modelo M-TAR (Momentum Threshold
Autoregressive), donde el régimen depende de la \textbf{dirección del
cambio} del desequilibrio en el periodo previo:

\begin{equation}
\Delta \mu_{i,c,t} 
= M_{i,c,t}\rho_1 \mu_{i,c,t-1} 
+ (1-M_{i,c,t})\rho_2 \mu_{i,c,t-1} 
+ \varepsilon_{i,c,t},
\end{equation}

donde la indicadora se define como:

\begin{equation}
M_{i,c,t}=
\begin{cases}
1 & \text{si } \Delta \mu_{i,c,t-1}\ge \tau,\\
0 & \text{si } \Delta \mu_{i,c,t-1}< \tau.
\end{cases}
\end{equation}

En aplicaciones empíricas suele fijarse \(\tau=0\), aunque también puede
estimarse de manera consistente siguiendo el procedimiento de Chan
(1993).

El modelo M-TAR permite capturar ajustes asimétricos asociados al
\emph{momentum} del desequilibrio: por ejemplo, reacciones distintas
cuando la brecha mayorista--minorista se está ampliando
(\(\Delta\mu_{i,c,t-1}\ge 0\)) frente a cuando se está cerrando
(\(\Delta\mu_{i,c,t-1}<0\)). La cointegración con ajuste M-TAR se
verifica cuando \(\rho_1<0\) y \(\rho_2<0\).
