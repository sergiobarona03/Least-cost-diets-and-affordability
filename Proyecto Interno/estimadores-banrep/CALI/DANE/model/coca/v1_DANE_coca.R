...
for (k in seq_along(escenarios)) {
  ...
  for (t in seq_along(fechas_k)) {
    ...
    df.aux <- df.aux %>% group_by(Food) %>%
      summarise(Price_100g = mean(Price_100g, na.rm = TRUE),
                Serving = 100,
    ...
    tryCatch({
      coca.aux <- FoodpriceR::CoCA(data = df.aux %>% filter(Price_100g > 0 &
                                                              Energy > 0), EER = EER)
...
# Combinar resultados
resultados_coca <- do.call(rbind, resultados[[1]])

# --- Preparar datos ---
resultados_coca <- resultados_coca %>%
  mutate(
    escenario = as.factor(escenario),
...
plot_coca_band <- function(data, sexo, age) {
  ...
  ggplot(data_wide, aes(x = fecha, group = Demo_Group)) +
    geom_line(aes(y = precio_100g), color = "red", linetype = 2, size = 0.6) +
    facet_wrap(~ Demo_Group, scales = "free_y", ncol = 3) +
    scale_x_date(date_labels = "%Y-%m", date_breaks = "6 months") +
    labs(
      title = paste("CoCA - Evolución del costo diario (", 
                    sexo, " - ", age ,")", sep = ""),
      x = "Fecha",
      y = "Costo diario (COP)"
    ) +
