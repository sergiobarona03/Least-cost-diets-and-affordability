...
input_cali_hat <- read.csv("estimadores-banrep/CALI/DANE/input/v4/v4_comp_price_data_cali.csv")
...
writexl::write_xlsx(resultados_cona %>% na.omit(), "estimadores-banrep/CALI/DANE/output/v4/v4_dane_cona_cali.xlsx")
writexl::write_xlsx(resultados_cona_comp %>% na.omit(), "estimadores-banrep/CALI/DANE/output/v4/v4_dane_cona_cali_comp.xlsx")
...
for (sx in levels(resultados_cona$Sex)) {
  for (agx in resultados_cona %>% filter(Sex == sx) %>% 
    ...
    ggsave(plot_cona_band(resultados_cona,sx, agx),
           filename = paste0("estimadores-banrep/CALI/DANE/output/v4/cona_",
...
ggsave(plot_cona_wide,
       filename = "estimadores-banrep/CALI/DANE/output/v4/v4_cona_1000kcal.png", 
       dpi = 300)