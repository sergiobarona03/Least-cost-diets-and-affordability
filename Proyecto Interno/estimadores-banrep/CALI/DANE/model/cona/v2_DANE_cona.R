#######################################
## Prueba: primera estimación CoNA   ##
#######################################
...
for (sx in levels(resultados_cona$Sex)) {
  for (agx in resultados_cona %>% filter(Sex == sx) %>% 
    ...
    ggsave(plot_cona_band(resultados_cona,sx, agx),
           filename = paste0("estimadores-banrep/CALI/DANE/output/v2/cona_",
...
ggsave(plot_cona_wide,
       filename = "estimadores-banrep/CALI/DANE/output/v2/v2_cona_1000kcal.png", 
       dpi = 300)
