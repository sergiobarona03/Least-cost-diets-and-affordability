...
sipsa_cona <- read_excel("estimadores-banrep/CALI/Resultados-10-2025/cona/010825_cona_cali.xlsx")
...
for (sx in levels(as.factor(overall_cona$Sex))) {
  ...
  for (agx in overall_cona %>% filter(Sex == sx) %>% 
    ...
    ggsave(
      filename = paste0("estimadores-banrep/CALI/Resultados-10-2025/validar/cona/cona_comparacion_", sx,"_",agx, ".png"),
      plot = p,
      width = 16,
      height = 12,
      dpi = 300
