...
sipsa_coca = readxl::read_excel("estimadores-banrep\\CALI\\Resultados-10-2025\\coca\\010825_coca_cali.xlsx")
dane_coca = readxl::read_excel("estimadores-banrep/CALI/Resultados-10-2025/estimadores-DANE/coca/291025_dane_coca_cali.xlsx")
...
for (sx in levels(as.factor(overall_coca$Sex))) {
  ...
  for (agx in overall_coca %>% filter(Sex == sx) %>% 
    ...
    ggsave(
      filename = paste0("estimadores-banrep/CALI/Resultados-10-2025/validar/coca/coca_comparacion_", sx,"_",agx, ".png"),
      plot = p,
      width = 16,
      height = 12,
      dpi = 300
