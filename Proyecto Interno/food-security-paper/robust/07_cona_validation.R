########################################################
## SCRIPT 07 — CoNA validation (MADE)
##
## Reads:
##   cache/: dane.rds
##   price_forecasting/prices_validation_panel.csv
##   panel_city_month_food_1999_2025.rds  (nutrients + pc)
##   220326_agg_eer.xlsx
##
## Three scenarios:
...
##
## Writes to .../robust/:
##   cona_validation_cost.csv
##   cona_validation_global.csv
##   cona_validation_city.csv
##   cona_validation_member.csv
##   cona_validation_city_member.csv
##   cona_validation_results.xlsx
##   fig_cona_*.png
########################################################
...
message(sprintf("  val_panel: %d | DANE obs: %d",
...
rename_nutrients <- function(df) {
  df %>% rename(
    Energy = energia_kcal, Protein = proteina_g,
    Lipids = lipidos_g, Carbohydrates = carbohidratos_totales_g,
    VitaminC = vitamina_c_mg, Folate = folatos_mcg,
    VitaminA = vitamina_a_er, Thiamine = tiamina_mg,
    Riboflavin = riboflavina_mg, Niacin = niacina_mg,
    VitaminB12 = vitamina_b12_mcg, Magnesium = magnesio_mg,
    Phosphorus = fosforo_mg, Sodium = sodio_mg,
    Calcium = calcio_mg, Iron = hierro_mg, Zinc = zinc_mg)
}
...
message(sprintf(
  "  S1(BASE): %d | S2(S5): %d | S3(Observed): %d",
...
eer_ll_base <- FoodpriceR::EER_LL %>%
  mutate(Age = recode(Age, "31 a 50 años"="[31,51)",
                      "9 a 13 años" ="[10, 14)"))
...
# -----------------------------------------------------------------------
# 6. Standardise and compute MADE
# -----------------------------------------------------------------------
...
message(sprintf("  S3 cost range: %.0f – %.0f COP/day",
...
cona_wide <- cona_S3 %>%
  rename(cona_S3 = cona_cost) %>% select(-scenario) %>%
  inner_join(cona_S1 %>% rename(cona_S1 = cona_cost) %>% select(-scenario),
             by = join_keys) %>%
  inner_join(cona_S2 %>% rename(cona_S2 = cona_cost) %>% select(-scenario),
             by = join_keys) %>%
  mutate(
...
city_summary <- cona_wide %>%
  ...
  summarise(n_obs = n(),
            ...
            MADE_S2 = mean(abs_delta_S2, na.rm=TRUE),
            pct_S2_closer = mean(closer == "S2_S5", na.rm=TRUE) * 100,
            imp_pp = MADE_S1 - MADE_S2, .groups = "drop")
...
p_dist <- ggplot(delta_long,
  ...
  labs(title = "CoNA deviation from observed prices",
       subtitle = sprintf(
...
ts_data <- cona_wide %>%
  ...
  pivot_longer(c(cona_S1, cona_S2, cona_S3),
...
p_ts <- ggplot(ts_data,
  ...
  scale_color_manual(
    values = c("S3: Observed" = COL_S3, "S1: BASE" = COL_S1,
               "S2: S5 (D_city JS)" = COL_S2)) +
  scale_linetype_manual(
    values = c("S3: Observed" = "solid", "S1: BASE" = "dashed",
               "S2: S5 (D_city JS)" = "dashed")) +
  scale_linewidth_manual(
    values = c("S3: Observed" = 1.1, "S1: BASE" = 0.8,
               "S2: S5 (D_city JS)" = 0.8)) +
...
write_xlsx(
  list(cost_wide = cona_wide, global = global_summary %>%
         pivot_longer(everything(), names_to="metric", values_to="value"),
       by_city = city_summary, by_member = member_summary,
       by_city_member = city_member_summary),
  file.path(robust_dir, "cona_validation_results.xlsx"))

# Decision print
cat("\n", strrep("=", 60), "\n")
cat("  CoNA VALIDATION SUMMARY\n")
...
cat(sprintf("  S2 closer:      %.1f%% of obs\n\n",
            global_summary$pct_S2_closer))

if (global_summary$MADE_S2 < global_summary$MADE_S1) {
  cat("  → S5 IMPROVES CoNA accuracy over BASE.\n\n")
} else {
  cat("  → BASE and S5 produce similar CoNA accuracy.\n\n")
}
