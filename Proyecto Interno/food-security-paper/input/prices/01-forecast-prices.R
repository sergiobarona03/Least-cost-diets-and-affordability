...
make_date <- function(ano, mes_num) as.Date(sprintf("%04d-%02d-01",
                                                    as.integer(ano), as.integer(mes_num)))

# Si hay más de una subclase, nos quedamos con la subclase más
# frecuente
mode1 <- function(x) {
  x <- x[!is.na(x)]
...
# Función 1: asignar la subclase para cada alimento
# con base en las correlativas de producto y subclase
assign_subclase_for_series <- function(df_series,
                                       corr_subclase, 
                                       corr_producto) {
  tmp1 <- df_series %>%
  ...
  # Si no encuentra subclase o si la sublclase es ambigua
  # Entonces se utiliza la correlativa a nivel de artículo
  if (is.na(sub1) || length(unique(na.omit(tmp1$subclase))) > 1) {
    print("CUIDADO: HAY MÁS DE UNA SUBCLASE")
    tmp2 <- df_series %>%
...
# Función 2: Construir la serie del IPC a partir de la subclase asignada
build_ipc_series <- function(ipc, city_name, subclase_ipc) {
  ...
  ipc %>%
    filter(ciudad == city_name, 
           cod_subclase == subclase_ipc_code) %>%
    select(fecha, ipc) %>%
    arrange(fecha)
}

# Función 3: Estimación hacia adelante
forward_fill_with_ipc <- function(df_series, df_ipc) {

  # Identificar última fecha observada y último precio observado
  last_obs_date <- max(df_series$fecha[!is.na(df_series$precio_500g)],
                       na.rm = TRUE)
  ...
  # Mín de la serie y máx del IPC (rango de la muestra)
  d0 <- min(df_series$fecha, na.rm = TRUE)
  d1 <- max(df_ipc$fecha, na.rm = TRUE)
  full_dates <- tibble(fecha = seq.Date(from = d0, 
                                        to = d1, by = "month"))
  
  # Empalme de precios
  # Identificar valores observados vs. a imputar
  out <- full_dates %>%
    left_join(df_series %>% 
                select(fecha, precio_500g), by = "fecha") %>%
    left_join(df_ipc, by = "fecha") %>%
    arrange(fecha) %>%
    mutate(
      precio_obs = precio_500g,
      precio_hat = NA_real_,
      status = if_else(!is.na(precio_obs),
                       "observed", "imputed")
    ) %>%
  ...
  if (length(anchor_idx) != 1) {
    return(list(data = out, ok = FALSE, 
                issue = "No unique anchor date index"))
  }
  if (is.na(out$ipc[anchor_idx])) {
    return(list(data = out, ok = FALSE,
                issue = "IPC missing at anchor month"))
  }
  ...
  if (anchor_idx < nrow(out)) {
    ...
    for (k in (anchor_idx + 1):nrow(out)) {
      ...
      # Estimar: pt-1*(IPCt/IPCt-1)
      out$precio_hat[k] <- out$precio_hat[k - 1] * (out$ipc[k] / out$ipc[k - 1])
    }
  }
  
  # Completar los NAs del precio_hat
  out <- out %>%
    mutate(precio_hat = if_else(!is.na(precio_obs), 
...
# -----------------------------
# 6. Correr las funciones para cada ciudad
# -----------------------------
...
for (i in seq_len(nrow(series_keys))) {
  ...
  sub_info <- assign_subclase_for_series(df_series,
                                         corr_subclase,
                                         corr_producto)
  ...
  # Derivar la serie del IPC a partir de la subclase asignada
  ...
  ff <- forward_fill_with_ipc(df_series %>%
  ...
  # Guardar en la lista de outputs
  ...
  out_png <- file.path(out_dir, "price_plots",
                       safe_name(city_name)
                       , paste0(safe_name(food_name), ".png"))
  dir.create(file.path(out_dir, "price_plots",
                       safe_name(city_name)), recursive = TRUE,
             showWarnings = FALSE)
  ggsave(out_png, p, width = 10, height = 4, dpi = 200)
}

# -----------------------------
# 7. Unir outputs y guardar
# -----------------------------
...
write_csv(prices_extended, file.path(out_dir, 
                                     "prices_extended_city_article_month.csv"))
write_csv(mapping_table,   file.path(out_dir,
                                     "mapping_city_article_to_ipc_subclase.csv"))
write_csv(fail_df,         file.path(out_dir, 
                                     "failures_log.csv"))
