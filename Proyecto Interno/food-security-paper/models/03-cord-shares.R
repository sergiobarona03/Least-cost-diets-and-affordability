...
panel_a <- ggplot(shares_time,
  ...
  scale_y_continuous(labels = percent_format(accuracy = 1),
                     expand = c(0, 0)) +
  labs(
    subtitle = "A. Monthly cost share by food group, household member, and city",
  ...
  theme(
    ...
    legend.position = "none"          # legend shared via patchwork
  )
...
panel_b <- ggplot(shares_mean,
  ...
  scale_fill_manual(values = group_colors) +
  labs(
    subtitle = "B. Mean cost share over 2019–2024\n(all cities and members)",
    x = NULL, y = NULL
  ) +
...
##----------------------------------------------------------
## Figure 6: Combine panels with patchwork
##----------------------------------------------------------

fig6 <- (panel_a | panel_b) +
  plot_layout(widths = c(3, 1)) +
  plot_annotation(
    title   = "Figure 6. Food group shares in the daily cost of the recommended diet, 2019–2024",
    caption = paste0(
      "Note: Panel A shows monthly cost shares for each food group as a proportion of total daily\n",
      "diet cost, disaggregated by household member and city. Panel B shows the mean cost share\n",
      "averaged across all city–month observations and household members. Food group classification\n",
      "follows GABAS dietary guidelines."
    ),
    theme = theme(
...
ggsave(file.path(out_fig, "fig6_cord_cost_shares.png"),
       fig6, width = 15, height = 10, dpi = 300)
