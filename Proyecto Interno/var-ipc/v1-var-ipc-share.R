...
for (i in 1:length(city_i)) {
  for (j in 19:length(food_j)) {
    ...
    if (length(df.aux$precio_500g) == 0) {
      ...
      # Agregar ponderaciones
      share_ipc = readxl::read_excel("var-ipc\\ponderaciones-IPC.xlsx") %>% janitor::clean_names()
      
      share_ipc = share_ipc %>% filter(nivel == "Subclase") %>%
        mutate(share = total_ingresos, cod_subclase = codigo) %>%
        select(cod_subclase, share)
      
      # Normalizar según subclases
      share_ipc2 = share_ipc %>% filter(cod_subclase == share_ipc$cod_subclase[9])
      
      # Unir var_ipc y share_ipc
      var_ipc = var_ipc %>% left_join(share_ipc, by = "cod_subclase")
      ...
      if (length(var_ipc$numero_indice) == 0) {
        ...
        ipc_df = test.df %>% full_join(df_ipc, by = c("ano", "mes_num")) %>%
          ...
          arrange(fecha) %>%
          mutate(precio_hat = NA, gamma0 = NA)
        ...
        for (k in 2:nrow(ipc_df)) {
          if (is.na(ipc_df$precio_hat[k])) {
            ipc_df$gamma0[k] = (ipc_df$ipc[k] / ipc_df$ipc[k - 1])
            ipc_df$precio_hat[k] = ipc_df$precio_hat[k - 1] * (ipc_df$ipc[k] / ipc_df$ipc[k - 1])*0.98
          }
        }
        
        # Calcular la tasa de variación del conjunto de validación
        test.df$gamma = NA
        for (k in 2:nrow(test.df)) {
            test.df$gamma[k] = (test.df$precio_500g[k] / ipc_df$precio_500g[k - 1])
        }
        ...
        test.df2 = test.df %>% left_join(ipc_df[c("fecha", "ipc",
          ...
          select(fecha, nombre_ciudad, articulo, precio_500g,
                 precio_hat, gamma, gamma0) %>% mutate(share = gamma0/gamma)
