########################################################
## SCRIPT v1/03_models/02_cona.R
## Version v1: mismo script que el pipeline en produccion,
## leyendo el panel de precios desde v1/output/ en vez de
## output/, y escribiendo los resultados dentro de v1/03_models/.
## Las funciones auxiliares (CoNA_paper.R) y el EER/UL por hogar
## no dependen del metodo de precio, se reutilizan del proyecto real.
##
...
aux_dir       <- file.path(base_dir, "03_models/aux-functions")
household_dir <- file.path(base_dir, "02_dataprep/household eer")
cona_dir      <- file.path(base_dir, "v1/03_models/cona")

dir.create(cona_dir, recursive = TRUE, showWarnings = FALSE)
...
data_paper <- readRDS(file.path(v1_output_dir, "paneles/panel_mensual_cities_tcac.rds")) %>%
  # household_eer_ll / household_ul usan ciudad en mayúscula sin tilde
  # (ej. "BOGOTA"); se agrega una columna normalizada solo para el
  # cruce, sin tocar la columna "ciudad" original del panel.
  mutate(ciudad_norm = toupper(iconv(ciudad, from = "", to = "ASCII//TRANSLIT")))

household_eer_ll <- readRDS(file.path(household_dir, "household_eer_ll.rds"))
household_ul     <- readRDS(file.path(household_dir, "household_ul.rds"))
...
# -----------------------------------------------------------------------
# Loop: CoNA for each city x date
# -----------------------------------------------------------------------

dominios <- sort(unique(household_eer_ll$ciudad))
fechas   <- sort(unique(data_paper$fecha))

message(sprintf("Estimating CoNA: %d cities x %d dates...",
