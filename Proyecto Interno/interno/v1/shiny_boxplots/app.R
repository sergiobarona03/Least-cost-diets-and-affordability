########################################################
## SCRIPT v1/02_dataprep/03_shiny_boxplots/app.R
##
## Boxplots de precio por gramo bajo el metodo v1 (sku mas
## cercano a 500g/1000ml). Comparar contra la version en
## produccion en 02_dataprep/03_shiny_boxplots/.
##
## Reads: data_boxplots.rds
## Correr con: shiny::runApp("v1/shiny_boxplots")
########################################################
...
if (!file.exists(ruta_datos)) {
  ruta_datos <- file.path(
    ...
    "v1/shiny_boxplots/data_boxplots.rds"
  )
}
if (!file.exists(ruta_datos)) {
...
ui <- fluidPage(
  titlePanel("Boxplots precio por gramo (v1: más cercano a 500g)"),
      ...
      sidebarLayout(
        ...
        mainPanel(
          ...
          p(
            ...
            sprintf("\"En CoNA\" marca los alimentos que entran en la dieta óptima")
          ),
