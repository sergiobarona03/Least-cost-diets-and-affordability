########################################################
## SCRIPT 02_dataprep/eer/01-EER-ninos.R
## Procesamiento de los requerimientos energéticos en niños y
## adolescentes. El procedimiento consiste en calcular un
## promedio ponderado considerando la estructura poblacional
## del CNPV-2018, para las 13 ciudades del proyecto.
##
## Reads:  requirements_dir/eer-ninos-adultos.xlsx
##         requirements_dir/cnpv-2018-municipios.xlsx
## Writes: output_dir/230726_child_eer.xlsx
##         output_dir/230726_plot_eer_g<1-4>.jpg
########################################################
...
male_rien <- readxl::read_excel(
  file.path(requirements_dir, "eer-ninos-adultos.xlsx"),
  sheet = 1
) %>%
  mutate(sex = "male")

female_rien <- readxl::read_excel(
  file.path(requirements_dir, "eer-ninos-adultos.xlsx"),
  sheet = 2
) %>%
  mutate(sex = "female")
...
cnpv_mun <- readxl::read_excel(
  file.path(requirements_dir, "cnpv-2018-municipios.xlsx")
) %>%
...
writexl::write_xlsx(
  weight_eer,
  file.path(output_dir, "230726_child_eer.xlsx")
)
...
for (i in seq_along(city_groups)) {
  ...
  ggsave(
    filename = file.path(output_dir, paste0("230726_plot_eer_g", i, ".jpg")),
    plot     = p,
    height   = 8,
    width    = 12,
    dpi      = 300
