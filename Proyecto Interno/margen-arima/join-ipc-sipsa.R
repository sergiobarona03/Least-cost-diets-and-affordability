...
retail_99_18$precio <- as.numeric(retail_99_18$precio)
retail_99_18 <- retail_99_18 %>%
  dplyr::mutate(
    unidad = str_replace_all(tolower(unidad), "\\s", ""),
    precio_500g_ipc = case_when(
...
whole_18_mean <- whole_18 %>% group_by(Year, Month, cod_mun, Alimento) %>%
  summarise(precio_500g_sipsa = mean(precio_500g, na.rm = TRUE))
...
retail_whole_18 <- whole_18_mean[c("Year", "Month", "cod_mun", "Alimento", "retail", "precio_500g_sipsa")] %>%
  left_join(
    retail_99_18,
    by = c(
       "Year" = "ano",
